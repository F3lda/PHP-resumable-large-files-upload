...
function handleUpload() {
    ...
    $chunk_dir = $upload_destination."uploads/TEMP/" . str_replace(".", "_", $filename).'---'. $filesize .'/';
...
function handleDelete() {
    ...
    $deleteFile = $_GET['DELETE'] ?? $_GET['delete'] ?? '';
    
    if ($deleteFile && file_exists($upload_destination."uploads/TEMP/" . str_replace(".", "_", $deleteFile).'/')){
        // Remove parts files and directory
        $dir = $upload_destination."uploads/TEMP/" . str_replace(".", "_", $deleteFile).'/';
        $files = array_diff(scandir($dir), array('.','..'));
        foreach($files as $file) {
            unlink("$dir/$file");
        }
    ...
    // Get current URL without DELETE parameter
    $redirect_url = getCurrentUrlWithoutParam('DELETE');
    header('Location: ' . $redirect_url);
    exit;
}

function getCurrentUrlWithoutParam($param) {
...
function getCurrentUrlWithAction($action) {
    $url_parts = parse_url($_SERVER['REQUEST_URI']);
    $base_url = $url_parts['path'];
...
function showUploadForm() {
    ...
    // Get file lists
    $dir = realpath($upload_destination.'uploads/TEMP');
    $temp_files_list = [];
    $uploaded_list = [];
    
    if ($dir !== false) {
        $temp_files_list = dirToArray($dir);
        $temp_files_list = tempDirsSizes($temp_files_list, $upload_destination.'uploads/TEMP');
    }
    
    if (file_exists($upload_destination.'uploads')) {
        $uploaded_list = dirToArray($upload_destination.'uploads', 1);
    }
    
    $check_url = getCurrentUrlWithAction('check');
    $upload_url = getCurrentUrlWithAction('upload');
    $merge_url = getCurrentUrlWithAction('merge');
    $base_url = getCurrentUrlBase();
...
const BYTES_PER_CHUNK = 1024 * 1024 * <?php echo $bytes_per_chunk; ?>; // 1MB chunk sizes
var fileSlices = 0;
...
<div id="incompleted">
    ...
    <?php foreach($temp_files_list as $fl): ?>
        <li>
            <a href="<?php echo $base_url . (strpos($base_url, '?') !== false ? '&' : '?') . 'DELETE=' . urlencode($fl['full_name']); ?>">X</a> - <span><?=$fl['name']?> (<?=$fl['human_size']?>/<?=$fl['human_full_size']?>)</span>
            <br>
...
<div id="completed">
    <h2>Completed:</h2>
    <ul>
    <?php foreach($uploaded_list as $fl): ?>
        <?php if ($fl === "TEMP") continue; ?>
        <li><a href="<?php echo $upload_destination ?>uploads/<?=$fl?>"><?=$fl?></a></li>
    <?php endforeach; ?>
    </ul>
</div>
